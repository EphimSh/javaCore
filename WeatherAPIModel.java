...
public class WeatherAPIModel implements VöderModel {
    ...
    @Override
    public void getWeather(String city, Integer number) throws IOException {

//        switch (number) {
//            case 1 -> currentDayWeatherRequest(city);
//            case 2, 3, 4, 5, 6, 7, 8, 9, 10 -> FiveDayWeatherForecastOnNextDays(city, number);
//        }

        if(number == 1) {
            currentDayWeatherRequest(city);
        } else {
            WeatherForecastRequest(city, number);
        }

    }

    private void WeatherForecastRequest(String city, Integer number) throws IOException {
        ...
        HttpUrl url = new HttpUrl.Builder()
                ...
                .addQueryParameter(DAYS_QUERY_PARAM, detectDaysNum(number,detectCity(city)))
    ...
    private String detectDaysNum(Integer days, String city) throws IOException {
//        https://api.weatherapi.com/v1/forecast.json?key=27120ec828d145d4bba92345223010&q=Yakutsk&days=10&aqi=no&alerts=no
        HttpUrl httpUrl = new HttpUrl.Builder()
                .scheme(PROTOCOL)
                .host(BASE_HOST)
                .addPathSegment(VERSION)
                ...
                .addQueryParameter(KEY_QUERY_PARAM, API_KEY)
                .addQueryParameter(CITY_QUERY_PARAM, city)
                .addQueryParameter(DAYS_QUERY_PARAM, days.toString())
                .addQueryParameter(AQI_QUERY_PARAM, "no")
                .addQueryParameter(ALERTS_QUERY_PARAM, "no")
                .build();
        Request request = new Request.Builder()
                ...
                .addHeader("accept", "application/json")
                .build();
        Response DaysWeatherResponse = client.newCall(request).execute();
        String weatherResponse = DaysWeatherResponse.body().string();

        String daysStr = "" + mapper.readTree(weatherResponse).get("forecast").at("/forecastday").size();
